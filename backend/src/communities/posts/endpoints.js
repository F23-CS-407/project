...
export async function get_posts_by_community(req, res){
    ...
    if(!comm){
        res.status(400).send('A community is required');
        return;
    }
    ...
    Community.findById(comm).populate({
        path: 'posts',
...
export async function like_post(req, res){
  ...
  if(!post){
    res.status(400).send('Post not found, or internal server error');
    return;
  }
  ...
  res.status(401).send('User has already liked this post');
}
...
export async function remove_like_post(req, res){
  ...
  res.status(401).send('User did not already like this post');
}
